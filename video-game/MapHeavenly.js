...
function Background({}) {
  ...
  const cubeMap = useCubeTexture(list, { path: "/cubemaps/lake/" });
  cubeMap.mapping = CubeRefractionMapping;

  let texture = useTexture(`/texture/church-glass.jpg`);

  return (
    <group scale={20} position-y={0} position-z={0}>
      <mesh frustumCulled={false} rotation-x={Math.PI * -0.5 * 0.0}>
        <sphereBufferGeometry args={[100, 24, 24]}></sphereBufferGeometry>
        <meshBasicMaterial
          // color={"red"}
          map={texture}
          side={DoubleSide}
...
function MapFloor() {
  ...
  let { floor } = useMemo(() => {
    ...
    src.scale.set(0.013333, 0.013333, 0.013333);
